...
mavenEnv(jdk: 21) {
  stage('prep') {
    ...
    // Retrieve content generated by prep.sh from https://ci.jenkins.io/job/Tools/job/bom/job/prep-only job (https://github.com/jenkinsci/bom/blob/prep-only/Jenkinsfile)
    def prepArchive = "prep_${commit}_${env.CHANGE_FORK ?: 'jenkinsci'}${consumeIncrementals ? '_consume-incrementals' : ''}.tar.gz"
    try {
      echo "INFO: trying to copy ${prepArchive} from last successful 'Tools/bom/prep-only' on same revision"
      copyArtifacts(projectName: 'Tools/bom/prep-only', parameters: "ARCHIVE_NAME=${prepArchive}", selector: lastCompleted(), filter: prepArchive, fingerprintArtifacts: true)
    } catch (e) {
      // Don't retrigger a new prep-only build on retry
      if (prepBuildId) {
        echo "INFO: build ${prepBuildId} already started from a previous attempt, no need to restart a new one"
      } else {
        echo "INFO: starting build to prepare ${prepArchive} from 'Tools/bom/prep-only'"
        def prepBuild = build(job: 'Tools/bom/prep-only', parameters: [string(name: 'ARCHIVE_NAME', value: prepArchive)], waitForStart: true)
        prepBuildId = prepBuild.externalizableId
        prepBuildNumber = prepBuild.number
      }
      waitForBuild(runId: prepBuildId, propagate: true, propagateAbort: false)
      copyArtifacts(projectName: 'Tools/bom/prep-only', parameters: "ARCHIVE_NAME=${prepArchive}", selector: specific("${prepBuildNumber}"), filter: prepArchive, fingerprintArtifacts: true)
    }
    withEnv(["PREP_ARCHIVE=${prepArchive}"]) {
      sh 'tar -xzvf "${PREP_ARCHIVE}" && rm -v "${PREP_ARCHIVE}"'
      sh 'mkdir -p "${MVN_LOCAL_REPO}/io/jenkins/tools/" && mv -v mvn-local-repo-bom "${MVN_LOCAL_REPO}/io/jenkins/tools/bom"'
    }
