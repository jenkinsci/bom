...
# identical to jenkins.version when using overrideWar. If we do not explicitly pass in
# jenkins.version, then the jenkins.version defined in the plugin's pom.xml file will be used, which
# may not match the version of the WAR under test.
#
mkdir pct-work
pushd pct-work
...
popd
rm -rf pct-work
MAVEN_PROPERTIES+=":jenkins.version=${JENKINS_VERSION}:overrideWar=true:useUpperBounds=true"
...
#
# TODO https://github.com/jenkinsci/maven-hpi-plugin/pull/376
MAVEN_PROPERTIES+=:hpi-plugin.version=3.33-rc1299.2ef16a_56f489
